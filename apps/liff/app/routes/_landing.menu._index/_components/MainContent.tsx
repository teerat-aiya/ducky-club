...
import { Calendar, Users, MapPin, Clock, ArrowRight, Zap, Coffee, MessageSquare, Star as StarIcon } from 'lucide-react';
...
export function MainContent({ user, stats, upcomingBookings }: MainContentProps) {
    ...
    <div className="space-y-6 pb-24">
        ...
        <div className="relative z-10">
          <div className="flex items-center justify-between mb-1">
            <div>
              ...
              <h1 className="text-2xl md:text-3xl font-bold">{user.name} {greetingEmoji}</h1>
            </div>
            <div className="w-12 h-12 rounded-full bg-white bg-opacity-20 flex items-center justify-center">
              <Zap className="w-5 h-5 text-yellow-300" fill="currentColor" />
            </div>
          </div>
          <p className="text-indigo-100/90 text-sm mt-2 mb-4">Welcome back to your workspace! What&apos;s on your mind today?</p>
            ...
            <div className="bg-white dark:bg-gray-800/80 backdrop-blur-sm p-4 rounded-2xl shadow-sm hover:shadow-md transition-all duration-300 border border-gray-100 dark:border-gray-700/50 h-full hover:-translate-y-1">
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <p className="font-bold text-xl text-gray-800 dark:text-white">{stats.upcomingEvents} Events</p>
                </div>
            ...
            <div className="bg-white dark:bg-gray-800/80 backdrop-blur-sm p-4 rounded-2xl shadow-sm hover:shadow-md transition-all duration-300 border border-gray-100 dark:border-gray-700/50 h-full hover:-translate-y-1">
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <p className="text-sm text-gray-500 dark:text-gray-400 mb-1">Community</p>
                  <p className="font-bold text-xl text-gray-800 dark:text-white">{stats.communityMembers} Members</p>
                </div>
        ...
        <motion.div variants={item} className="col-span-2">
          <div className="bg-gradient-to-r from-amber-50 to-amber-50/70 dark:from-amber-900/20 dark:to-amber-900/10 p-4 rounded-2xl border border-amber-100 dark:border-amber-900/30">
            <div className="flex items-center">
              ...
              <div>
                <p className="font-medium text-amber-900 dark:text-amber-100">Available Spaces</p>
                <p className="text-2xl font-bold text-amber-700 dark:text-amber-300">{stats.spacesAvailable} Ready</p>
              </div>
            </div>
          </div>
        </motion.div>
      ...
      >
        <div className="flex items-center justify-between mb-5">
          <h2 className="font-bold text-lg text-gray-800 dark:text-white">Quick Actions</h2>
        ...
        <div className="grid grid-cols-4 gap-2">
          {[
            { icon: <Calendar className="w-5 h-5" />, label: 'Events', color: 'bg-amber-100 text-amber-600 dark:bg-amber-900/30 dark:text-amber-400', to: '/events' },
            { icon: <Users className="w-5 h-5" />, label: 'Community', color: 'bg-blue-100 text-blue-600 dark:bg-blue-900/30 dark:text-blue-400', to: '/community' },
            { icon: <Coffee className="w-5 h-5" />, label: 'Book Space', color: 'bg-green-100 text-green-600 dark:bg-green-900/30 dark:text-green-400', to: '/booking' },
            { icon: <MessageSquare className="w-5 h-5" />, label: 'Chat', color: 'bg-purple-100 text-purple-600 dark:bg-purple-900/30 dark:text-purple-400', to: '/chat' },
          ].map((action, index) => (
            <motion.div 
              key={action.label}
              whileTap={{ scale: 0.95 }}
            ...
            >
              <Link 
                to={action.to}
                className="flex flex-col items-center justify-center p-3 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors group"
              >
                <div className={`w-12 h-12 ${action.color} rounded-2xl flex items-center justify-center mb-2 group-hover:scale-110 transition-transform`}>
                  {action.icon}
                </div>
      ...
      <motion.div variants={item} className="space-y-1">
        <div className="flex items-center justify-between px-1">
          <h2 className="font-bold text-lg text-gray-800 dark:text-white">Upcoming Bookings</h2>
          <Link to="/booking" className="text-sm text-amber-600 dark:text-amber-400 font-medium flex items-center group">
            View all
        ...
        <AnimatePresence>
          {upcomingBookings.length > 0 ? (
              ...
              {upcomingBookings.map((booking, index) => (
                ...
                >
                  <Link
                    ...
                    className="block p-4 bg-white dark:bg-gray-800/80 backdrop-blur-sm rounded-2xl shadow-sm hover:shadow-md border border-gray-100 dark:border-gray-700/50 transition-all duration-300 group"
                  >
                    <div className="flex items-start">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start justify-between">
                          <h3 className="font-semibold text-gray-900 dark:text-white truncate">{booking.title}</h3>
            ...
            >
              ...
              <h3 className="font-medium text-gray-700 dark:text-gray-300">No upcoming bookings</h3>
              <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Book a space to get started</p>
              <Link 
                to="/booking" 
