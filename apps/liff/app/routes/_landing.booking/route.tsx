...
export const loader = async () => {
  ...
  const spaceTypes = [
    ...
    { value: 'desk', label: 'Desks', count: mockSpaces.filter(s => s.type === 'desk').length },
    { value: 'meeting_room', label: 'Meeting Rooms', count: mockSpaces.filter(s => s.type === 'meeting_room').length },
    { value: 'phone_booth', label: 'Phone Booths', count: mockSpaces.filter(s => s.type === 'phone_booth').length },
    { value: 'focus_pod', label: 'Focus Pods', count: mockSpaces.filter(s => s.type === 'focus_pod').length },
    { value: 'event_space', label: 'Event Spaces', count: mockSpaces.filter(s => s.type === 'event_space').length },
    { value: 'lounge', label: 'Lounges', count: mockSpaces.filter(s => s.type === 'lounge').length }
  ];

  const availableSpaces = mockSpaces.filter(s => s.isAvailable).length;
  const occupancyRate = Math.round(((mockSpaces.length - availableSpaces) / mockSpaces.length) * 100);
